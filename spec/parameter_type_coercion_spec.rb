  ...
  describe 'Integer' do
    ...
    it 'returns 400 on requests when integer is invalid' do
      get('/coerce/integer', arg: '123abc') do |response|
        ...
        expect(JSON.parse(response.body)['message']).to eq("Invalid value for parameter 'arg': '123abc' is not a valid Integer")
      end
    end
  end
  ...
  describe 'Float' do
    ...
    it 'returns 400 on requests when float is invalid' do
      get('/coerce/float', arg: '123abc') do |response|
        expect(response.status).to eql 400
        expect(JSON.parse(response.body)['message']).to eq("Invalid value for parameter 'arg': '123abc' is not a valid Float")
      end
    end
  end
  ...
  describe 'Time' do
    ...
    it 'returns 400 on requests when time is invalid' do
      get('/coerce/time', arg: '123abc') do |response|
        expect(response.status).to eql 400
        expect(JSON.parse(response.body)['message']).to eq("Invalid value for parameter 'arg': '123abc' is not a valid Time")
      end
    end
  end
  ...
  describe 'Date' do
    ...
    it 'returns 400 on requests when date is invalid' do
      get('/coerce/date', arg: 'abc') do |response|
        expect(response.status).to eql 400
        expect(JSON.parse(response.body)['message']).to eq("Invalid value for parameter 'arg': 'abc' is not a valid Date")
      end
    end
  end
  ...
  describe 'DateTime' do
    ...
    it 'returns 400 on requests when datetime is invalid' do
      get('/coerce/datetime', arg: 'abc') do |response|
        expect(response.status).to eql 400
        expect(JSON.parse(response.body)['message']).to eq("Invalid value for parameter 'arg': 'abc' is not a valid DateTime")
      end
    end
  end
